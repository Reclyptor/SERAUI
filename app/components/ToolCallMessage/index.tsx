...
export function ToolCallMessage({ toolCall, isLatest }: ToolCallMessageProps) {
  ...
  const resultStr =
    toolCall.result != null ? formatResult(toolCall.result) : null;
  ...
  return (
    <div className="my-1">
      ...
      >
        <DotIcon
          className={`w-1.5 h-1.5 flex-shrink-0 ${isActive ? "text-accent animate-pulse" : "text-foreground-muted/50"}`}
        />
      ...
      {hasDetail && (
        <Collapsible isOpen={!isCollapsed}>
          ...
          >
            {argsStr && (
              <pre className="text-foreground-muted/60 max-h-[120px] overflow-y-auto">
                {argsStr}
              </pre>
            )}
            {resultStr && (
